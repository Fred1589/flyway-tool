...
public class MigrationService extends AbstractMigrationService {
	...
	public void execute() {
		...
		final Flyway flyway = new FlywayBuilder().withDataSource(getSchemaDataSource())
				.withEnviromentProperties(getProperties()).withPlaceHolders(getPlaceHolders())
				.withSchema(getProperties().getSchemaName()).build();
	...
	private Map<String, String> getPlaceHolders() {
		...
		placeholders.put("schemaName", getProperties().getSchemaName());
		return placeholders;
	}
